$(window).ready(function () {
    ...
    fetch("../static/data/data.json").then(res => res.json()).then(data => {
        ...
        $(".comment-form button[type=submit]").on("click", function () {
            let commentName = $(".comment-field input[type=text]").val();
            let commentEmail = $(".comment-field input[type=email]").val();
            let commentContent = $(".comment-field textarea").val();
            ...
            if (commentName === "") {
                ...
                //Lấy rating hiện tại ra
                const stars = $(".comment-form .stars").html();
                //Thêm comment mới
                $(".comment-posts").append(`
                    <div class="comment-post">
                            <div class="avatar">
                                <img src="../static/images/avatar1.avif">
                            </div>
                    ...
                    `);
                //Reset lại khung comment
                $(".comment-field input[type=text]").val('');
                $(".comment-field input[type=email]").val('');
        ...
        //Thay đổi star
        for (let i = 1; i <= 5; i++) {
            $(`.stars i:nth-child(${i})`).click(function () {
                $(".comment-form .stars i").removeClass("active");
                for (let j = 1; j <= i; j++) {
                    $(`.comment-form .stars i:nth-child(${j})`).addClass("active");
                }
            });
        }
    });
